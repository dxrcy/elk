...
pub fn halt(_: *Runtime, _: ?*const anyopaque) Traps.Result {
    return error.Halt;
}

pub fn getc(runtime: *Runtime, _: ?*const anyopaque) Traps.Result {
    return readChar(runtime, .getc);
}

pub fn in(runtime: *Runtime, _: ?*const anyopaque) Traps.Result {
...
pub fn out(runtime: *Runtime, _: ?*const anyopaque) Traps.Result {
    const word: u8 = @truncate(runtime.registers[0]);
    try runtime.writer.interface.writeByte(word);
    try runtime.writer.interface.flush();
}

pub fn puts(runtime: *Runtime, _: ?*const anyopaque) Traps.Result {
...
pub fn putsp(runtime: *Runtime, _: ?*const anyopaque) Traps.Result {
    var i: usize = runtime.registers[0];
    while (true) : (i += 1) {
...
pub fn putn(runtime: *Runtime, _: ?*const anyopaque) Traps.Result {
    try runtime.writer.ensureNewline();
    try runtime.writer.interface.print("{}\n", .{runtime.registers[0]});
    try runtime.writer.interface.flush();
}

pub fn reg(runtime: *Runtime, _: ?*const anyopaque) Traps.Result {
