...
pub const Diagnostic = union(enum) {
    ...
    pub fn getPrimarySpan(diag: Diagnostic) ?Span {
        return switch (diag) {
            ...
            .multiple_origins => |info| info.existing,
            ...
            .label_colon => |info| info.colon,
            .redefined_label => |info| info.existing,
            .undefined_label => |info| info.reference,
            .unused_label => |info| info.label,
            .label_too_long => |info| info.label,
            .breakpoint_label => |info| info.label,
            .malformed_integer => |info| info.integer,
            .malformed_character => |info| info.integer,
            .expected_digit => |info| info.integer,
            .invalid_digit => |info| info.integer,
            .unexpected_delimiter => |info| info.integer,
            .nonstandard_integer_radix => |info| info.integer,
            .implicit_integer_radix => |info| info.integer,
            .nonstandard_integer_form => |info| info.integer,
            .undesirable_integer_form => |info| info.integer,
            .character_integer => |info| info.integer,
            .integer_too_large => |info| info.integer,
            .offset_too_large => |info| info.reference,
            .unexpected_negative_integer => |info| info.integer,
            ...
            .emulate_exception => null,
            .debugger_requires_assembler => |info| info.command,
            .debugger_requires_file => |info| info.command,
            .debugger_requires_assembly => |info| info.command,
            .debugger_requires_symbols => |info| info.command,
            .debugger_address_not_in_assembly => null,
            .debugger_address_not_user_memory => |info| info.address,
            .debugger_label_partial_match => |info| info.reference,
            .debugger_no_space => null,
            .debugger_invalid_argument_kind => |info| info.found,
            .debugger_invalid_command => |info| info.command,
            .debugger_missing_subcommand => |info| info.eol,
            .debugger_unexpected_eol => |info| info.eol,
            .debugger_expected_eol => |info| info.found,
            .debugger_integer_too_small => |info| info.integer,
            .symbol_not_found => |info| info.symbol,
        };
    }
};
